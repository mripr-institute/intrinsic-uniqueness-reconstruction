...
def reconstruct():
    ...
    additional_totals = Counter(item["status"] for item in additional)
    return {
        "scope": "All named mathematical environments in the current paper. "
                 ...
                 "The named count includes definitions; additional labelled claims are "
                 "tracked separately. Complete Lean coverage is not yet established.",
        "count": len(items),
        "named_statement_count": len(items) - totals["definition"],
        "totals": dict(totals),
        "additional_labelled_claim_totals": dict(additional_totals),
...
def markdown(ledger):
    ...
    extra = ledger["additional_labelled_claim_totals"]
    lines = ["# Current paper-to-Lean formalization coverage", "", ledger["scope"], "",
             ...
             f"Additional labelled claims (outside named environments): "
             f"{len(ledger['additional_labelled_claims'])}; complete: {extra.get('complete', 0)}; "
             f"partial: {extra.get('partial', 0)}; missing: {extra.get('missing', 0)}.", "",
...
def inventory_markdown(ledger):
    ...
    lines = ["# Inventory of Lean-formalized paper results", "",
             ...
             f"- Additional labelled claims outside named environments: "
             f"{len(additional)} ({extra.get('complete', 0)} complete, "
             f"{extra.get('partial', 0)} partial, {extra.get('missing', 0)} missing).",
...
def main():
    ...
    extra = ledger["additional_labelled_claim_totals"]
    print(f"ADDITIONAL LABELLED CLAIMS: {sum(extra.values())} "
          f"({extra.get('complete', 0)} complete, {extra.get('partial', 0)} partial, "
          f"{extra.get('missing', 0)} missing)")
